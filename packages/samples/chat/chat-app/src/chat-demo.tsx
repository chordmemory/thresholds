...
import { ChatManifest } from './types';

export const ChatDemo = ({ chatApi }: { chatApi: ChatApi }) => {
  const [chatList, setChatList] = React.useState<ChatListType>([]);
  const [userId, setUserId] = React.useState<string>('unknown');
  ...
  const createChat = async (displayName: string) => {
  ...
  const onChatOpened = async (chatId: string) => {
  ...
  return (
    <>
      <h2>User Details</h2>
      <UserManager onIdChanged={setUserId} />
      <h2>Chat List</h2>
      <ChatList
        chats={chatList}
      ...
      />
      <h3>Chat Creator</h3>
      {userId && <ChatCreator onCreateChat={createChat} />}
      <h3>Open Chats</h3>
      {openChats.map((chat) => (
        <ChatBox
          key={chat.chatId}
          ...
          onSubmitMessage={(body) =>
            chatApi.addMessage(chat.chatId, {
              body,
              userId: userId
            })
          }
