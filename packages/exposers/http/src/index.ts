...
export interface HttpExposeOptions {
  route: string;
  method: string;
  paramMapper?: (params: any, body?: any[]) => unknown[];
}

const defaultParamMapper = (params: { [param: string]: any; rest?: any[] }) => {
  const namedParams = {
    ...params,
    rest: undefined
  };
      ...
      get: () => {
        app.get(route, async (req: express.Request, res: express.Response) => {
          ...
          try {
            ...
            console.log(
              `Unexpected error executing function ${config.name}`,
              error
            );
            res
              .status(500)
              .send(
                'Unexpected error executing function. See server logs for info'
              );
          }
        });
      },
